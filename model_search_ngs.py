...
"""
    NGS Search Space
    Paper: Explainable Graph-based Fraud Detection via Neural Meta-graph Search
...
class Op(nn.Module):
    ...
    def forward(self, x, adjs, ws, idx):
        ...
        for i in range(ws.size(0)):
            fin.append(ws[i] * torch.spmm(adjs[i], x))
        return sum(fin)
...
class Model(nn.Module):
    ...
    def forward(self, node_feats, node_types, adjs, idxes_seq, idxes_res):
        node_embeddings = self.embedding(node_feats, node_types, adjs, idxes_seq, idxes_res)
