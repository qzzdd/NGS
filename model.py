...
"""
    Architecture derived from the search space.
    Modified from https://github.com/LARS-research/DiffMG
"""
...
class Model(nn.Module):
    ...
    def __init__(self, in_dim, n_hid, num_node_types, n_classes, n_steps, dropout=None, attn_dim=64, use_norm=True, out_nl=True, in_nl=False):
        ...
        self.train_valid_neg_idx = None
        self.train_valid_pos_idx = None
    ...
    def forward(self, node_feats, node_types, adjs, idxes_seq, idxes_res):
        out = self.embedding(node_feats, node_types, adjs, idxes_seq, idxes_res)
        logits = self.classifier(out)
        return logits

    def embedding(self, node_feats, node_types, adjs, idxes_seq, idxes_res):
        ...
        if self.initialize:
            attns = self.__init_attns__(attns)
            self.initialize = False
    ...
    def __init_attns__(self, attns):
        for i in self.train_valid_neg_idx:
            if attns[i][0]:
                continue
            else:
                attns[i][0] += 1.
                attns[i][1] -= 1.
        for i in self.train_valid_pos_idx:
            if attns[i][1]:
                continue
            else:
                attns[i][1] += 1.
    ...
    def init_index(self, train_idx, train_target, valid_idx, valid_target):
        self.train_valid_neg_idx = torch.cat((train_idx[train_target == 0], valid_idx[valid_target == 0]), dim=-1).numpy()
        self.train_valid_pos_idx = torch.cat((train_idx[train_target == 1], valid_idx[valid_target == 1]), dim=-1).numpy()
